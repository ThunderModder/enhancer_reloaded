...
public class ItemDefHPHeal extends ItemModifier {
	...
	public ItemDefHPHeal(String ulocalizedName, EnumModifierType type) {
	...
	public void getDescription(ItemStack stack, List<String> tips){
		...
		tips.add(TextFormatting.DARK_GRAY + I18n.format("tooltip.energy", new Object[]{}) + TextFormatting.DARK_PURPLE + Constants.DEF_HPHEAL_BASE * NBTItemProvider.getPower(stack) + " LT");
    }
	
	@Override
	public void onInventoryTick(EntityPlayer player, ItemStack stack) {

		if(player.getHealth() <= player.getMaxHealth() * 0.3F && NBTItemProvider.discharge(stack, Constants.DEF_HPHEAL_BASE)){
			player.heal(NBTItemProvider.getPower(stack));
		}
