...
public class ItemUTSpeedModifier extends ItemModifier {
		
	public ItemUTSpeedModifier(String ulocalizedName, EnumModifierType type) {
	...
	public void getDescription(ItemStack stack, List<String> tips){
		...
		tips.add(TextFormatting.DARK_GRAY + I18n.format("tooltip.energy", new Object[]{}) + TextFormatting.DARK_PURPLE + Constants.UT_SPEED_BASE * NBTItemProvider.getPower(stack) + " LT");
    }
	
	@Override
	public void onInventoryTick(EntityPlayer player, ItemStack stack) {
	
		if(NBTItemProvider.discharge(stack, Constants.UT_SPEED_BASE)){
			Utils.addPotionEffect(player, Utils.POTION_SPEED, NBTItemProvider.getPower(stack));
		}
