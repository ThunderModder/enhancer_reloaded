...
public class ItemUTWbModifier extends ItemModifier {
	
	public ItemUTWbModifier(String ulocalizedName, EnumModifierType type) {
	...
	public void getDescription(ItemStack stack, List<String> tips){
		...
		tips.add(TextFormatting.DARK_GRAY + I18n.format("tooltip.energy", new Object[]{}) + TextFormatting.DARK_PURPLE + Constants.UT_WB_BASE * NBTItemProvider.getPower(stack) + " LT");
    }
	
	@Override
	public void onInventoryTick(EntityPlayer player, ItemStack stack) {
	
		if(player.isInWater() && NBTItemProvider.discharge(stack, Constants.UT_WB_BASE)){
			Utils.addPotionEffect(player, Utils.POTION_WBREATHING, NBTItemProvider.getPower(stack));
		}
