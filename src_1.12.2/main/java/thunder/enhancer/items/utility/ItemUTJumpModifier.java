...
public class ItemUTJumpModifier extends ItemModifier {
	
	public ItemUTJumpModifier(String ulocalizedName, EnumModifierType type) {
	...
	public void getDescription(ItemStack stack, List<String> tips){
		...
		tips.add(TextFormatting.DARK_GRAY + I18n.format("tooltip.energy", new Object[]{}) + TextFormatting.DARK_PURPLE + Constants.UT_JUMP_BASE * NBTItemProvider.getPower(stack) + " LT");
    }
	
	@Override
	public void onInventoryTick(EntityPlayer player, ItemStack stack) {

		if(NBTItemProvider.discharge(stack, Constants.UT_JUMP_BASE)){
			Utils.addPotionEffect(player, Utils.POTION_JUMPB, NBTItemProvider.getPower(stack));
		}
