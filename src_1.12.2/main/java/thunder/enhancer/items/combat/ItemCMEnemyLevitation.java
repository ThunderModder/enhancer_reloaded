...
public class ItemCMEnemyLevitation extends ItemModifier {

	public ItemCMEnemyLevitation(String ulocalizedName, EnumModifierType type) {
	...
	public void getDescription(ItemStack stack, List<String> tips){
		...
		tips.add(TextFormatting.DARK_GRAY + I18n.format("tooltip.energy", new Object[]{}) + TextFormatting.DARK_PURPLE + Constants.CM_ENEMYLEVITATION_BASE * NBTItemProvider.getPower(stack) + " LT");
    }
	
	@Override
	public void onAttack(LivingAttackEvent event, EntityPlayer source, EntityLivingBase target, ItemStack stack) {
		
		if(NBTItemProvider.discharge(stack, Constants.CM_ENEMYLEVITATION_BASE)){
			Utils.addPotionEffect(target, Utils.POTION_LEVITATION, 1, NBTItemProvider.getPower(stack) * 100);
		}
	}
