...
public class ItemCMAttackVampirism extends ItemModifier {

	public ItemCMAttackVampirism(String ulocalizedName, EnumModifierType type) {
	...
	public void getDescription(ItemStack stack, List<String> tips){
		...
		tips.add(TextFormatting.DARK_GRAY + I18n.format("tooltip.energy", new Object[]{}) + TextFormatting.DARK_PURPLE + Constants.CM_ATTACKVAMPIRISM_BASE * NBTItemProvider.getPower(stack) + " LT");
    }
	
	@Override
	public void onAttack(LivingAttackEvent event, EntityPlayer source, EntityLivingBase target, ItemStack stack) {
		
		if(Utils.getRandom(15) && NBTItemProvider.discharge(stack, Constants.CM_ATTACKVAMPIRISM_BASE)){
			float health = target.getHealth() - NBTItemProvider.getPower(stack);
			
			source.heal(NBTItemProvider.getPower(stack));
			if(health > 0) target.setHealth(health);
			else target.attackEntityFrom(DamageSource.GENERIC, 1000.0f);
		}
	}
