...
public class ItemCMKillOrDie extends ItemModifier {

	public ItemCMKillOrDie(String ulocalizedName, EnumModifierType type) {
	...
	public void getDescription(ItemStack stack, List<String> tips){
		...
		tips.add(TextFormatting.DARK_GRAY + I18n.format("tooltip.energy", new Object[]{}) + TextFormatting.DARK_PURPLE + Constants.CM_KILLORDIE_BASE * NBTItemProvider.getPower(stack) + " LT");
    }
	
	@Override
	public void onAttack(LivingAttackEvent event, EntityPlayer source, EntityLivingBase target, ItemStack stack) {
		
		if(NBTItemProvider.discharge(stack, Constants.CM_KILLORDIE_BASE)){
			if(Utils.getRandom(50)){
				target.attackEntityFrom(DamageSource.GENERIC, 1000.0f);
			}else{
				source.attackEntityFrom(DamageSource.GENERIC, 1000.0f);
			}
		}
	}
