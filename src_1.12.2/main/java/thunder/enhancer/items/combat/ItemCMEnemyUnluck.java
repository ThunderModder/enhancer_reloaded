...
public class ItemCMEnemyUnluck extends ItemModifier {

	public ItemCMEnemyUnluck(String ulocalizedName, EnumModifierType type) {
	...
	public void getDescription(ItemStack stack, List<String> tips){
		...
		tips.add(TextFormatting.DARK_GRAY + I18n.format("tooltip.energy", new Object[]{}) + TextFormatting.DARK_PURPLE + Constants.CM_ENEMYUNLUCK_BASE * NBTItemProvider.getPower(stack) + " LT");
    }
	
	@Override
	public void onAttack(LivingAttackEvent event, EntityPlayer source, EntityLivingBase target, ItemStack stack) {
		
		if(NBTItemProvider.discharge(stack, Constants.CM_ENEMYUNLUCK_BASE)){
			Utils.addPotionEffect(target, Utils.POTION_UNLUCK, NBTItemProvider.getPower(stack), 60);
		}
	}
