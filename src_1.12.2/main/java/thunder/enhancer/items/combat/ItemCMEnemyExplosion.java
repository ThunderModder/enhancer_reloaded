...
public class ItemCMEnemyExplosion extends ItemModifier {

	public ItemCMEnemyExplosion(String ulocalizedName, EnumModifierType type) {
	...
	public void getDescription(ItemStack stack, List<String> tips){
		...
		tips.add(TextFormatting.DARK_GRAY + I18n.format("tooltip.energy", new Object[]{}) + TextFormatting.DARK_PURPLE + Constants.CM_ENEMYEXPLOSION_BASE * NBTItemProvider.getPower(stack) + " LT");
    }
	
	@Override
	public void onAttack(LivingAttackEvent event, EntityPlayer source, EntityLivingBase target, ItemStack stack) {
		
		if(NBTItemProvider.discharge(stack, Constants.CM_ENEMYEXPLOSION_BASE)){
			target.getEntityWorld().createExplosion(target, target.posX, target.posY, target.posZ, NBTItemProvider.getPower(stack), false);
		}
	}
