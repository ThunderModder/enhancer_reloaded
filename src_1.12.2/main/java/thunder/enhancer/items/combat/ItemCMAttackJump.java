...
public class ItemCMAttackJump extends ItemModifier {

	public ItemCMAttackJump(String ulocalizedName, EnumModifierType type) {
	...
	public void getDescription(ItemStack stack, List<String> tips){
		...
		tips.add(TextFormatting.DARK_GRAY + I18n.format("tooltip.energy", new Object[]{}) + TextFormatting.DARK_PURPLE + Constants.CM_ATTACKJUMP_BASE * NBTItemProvider.getPower(stack) + " LT");
    }
	
	@Override
	public void onAttack(LivingAttackEvent event, EntityPlayer source, EntityLivingBase target, ItemStack stack) {
		
		if(NBTItemProvider.discharge(stack, Constants.CM_ATTACKJUMP_BASE)){
			Utils.addPotionEffect(source, Utils.POTION_JUMPB, NBTItemProvider.getPower(stack));
		}
	}
