...
public class ItemCMEnemyPoison extends ItemModifier {

	public ItemCMEnemyPoison(String ulocalizedName, EnumModifierType type) {
	...
	public void getDescription(ItemStack stack, List<String> tips){
		...
		tips.add(TextFormatting.DARK_GRAY + I18n.format("tooltip.energy", new Object[]{}) + TextFormatting.DARK_PURPLE + Constants.CM_ENEMYPOISON_BASE * NBTItemProvider.getPower(stack) + " LT");
    }
	
	@Override
	public void onAttack(LivingAttackEvent event, EntityPlayer source, EntityLivingBase target, ItemStack stack) {
		
		if(NBTItemProvider.discharge(stack, Constants.CM_ENEMYPOISON_BASE)){
			Utils.addPotionEffect(target, Utils.POTION_POISON, NBTItemProvider.getPower(stack), 60);
		}
	}
