...
public class ItemCMAttackAbsorption extends ItemModifier {

	public ItemCMAttackAbsorption(String ulocalizedName, EnumModifierType type) {
	...
	public void getDescription(ItemStack stack, List<String> tips){
		...
		tips.add(TextFormatting.DARK_GRAY + I18n.format("tooltip.energy", new Object[]{}) + TextFormatting.DARK_PURPLE + Constants.CM_ATTACKABSORPTION_BASE * NBTItemProvider.getPower(stack) + " LT");
    }
	
	@Override
	public void onAttack(LivingAttackEvent event, EntityPlayer source, EntityLivingBase target, ItemStack stack) {
		
		if(NBTItemProvider.discharge(stack, Constants.CM_ATTACKABSORPTION_BASE)){
			Utils.addPotionEffect(source, Utils.POTION_ABSRORPTION, NBTItemProvider.getPower(stack), 40);
		}
	}
